# CodeFire MCP — Gemini CLI Setup

## Step 1: Connect the MCP Server

Add the CodeFire MCP server to your Gemini CLI settings file. CodeFire must be installed and running.
...
Add to `%APPDATA%\.gemini\settings.json`:
```json
{
  "mcpServers": {
    "codefire": {
...
Start a new Gemini CLI session and ask: "What MCP tools do you have from codefire?" — you should see 60+ tools listed.

## Step 2: Add Instructions to GEMINI.md

Copy the section below into your `~/.gemini/GEMINI.md` file (global instructions). This tells Gemini CLI how to use the CodeFire tools effectively.
