# CodeFire MCP — Codex CLI Setup

## Step 1: Connect the MCP Server

Add the CodeFire MCP server to your Codex CLI config. CodeFire must be installed and running.
...
Add to `%APPDATA%\.codex\config.toml`:
```toml
[mcp_servers.codefire]
command = "node"
...
Start a new Codex CLI session and ask: "What MCP tools do you have from codefire?" — you should see 60+ tools listed.

## Step 2: Add Instructions to AGENTS.md

Copy the section below into your `~/.codex/AGENTS.md` file (global instructions). This tells Codex CLI how to use the CodeFire tools effectively.
